...
| 來源 | 欄位 | 說明 |
|------|------|------|
| AI | questions[] | 索克拉底六類提問（澄清/假設/證據/觀點/後果/反思） |
| 使用者 | answers[] | 使用者回答 |
...
| 端點 | 功能 | 邊界 |
|------|------|------|
| `POST /causal-loops/generate` | 從 TaskDefinition + 索克拉底問答 + 矛盾句中自動抽取因果節點/邊，建立因果迴路圖並識別斷路點 | AI 建模初稿，人工審核/修改/補充 |

**AI 建模邏輯**：
- 從索克拉底回答中抽取因果關係（e.g. 「轉速提高會導致 NVH 惡化」→ 邊 `轉速 --+→ NVH`）
...
| 端點 | 功能 | 邊界 |
|------|------|------|
| `POST /contradictions/identify` | 從索克拉底回答中識別矛盾 | AI 初識別，人工審核/修改 |
...
無直接 AI 生成。假設由 RD 工程師根據 Step 2 索克拉底問答與工程經驗填入。
...
| socratic_questions.md | 生成索克拉底問題 | Step 2 |
