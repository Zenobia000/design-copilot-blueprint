...
> **原獨立文件，現為 E3 附錄。**
> 原檔：docs/01-define/architecture/Forward_Subsystem_Discovery_Architecture.md
...
> **原獨立文件，現為 E3 附錄。**
> 原檔：docs/01-define/architecture/Reverse_Anti_Anchor_Architecture.md
> 錨點：`#appendix-c-reverse-anti-anchor-architecture`

## 反向探索・Anti-Anchor：系統架構說明書（SA 視角）

> **版本**：v1.0 | **日期**：2026-04-08 | **觀點**：Systems Analyst
> **對齊依據**：`AI_Agent_Architecture.md` v1.4、`RD_Design_Copilot_整合流程.md` v1.6、`Forward_Subsystem_Discovery_Architecture.md` v2.0
> **文件目的**：以 SA 視角拆解「反向探索・Anti-Anchor」階段（F0 反向路徑）的所有架構面向——actors、use cases、context、container、component、data、sequence、state、deployment——每張圖以 Mermaid 呈現，供開發、測試、維運與後續迭代共同對齊。
> **本文件與 Forward 文件的關係**：`Forward_Subsystem_Discovery_Architecture.md` 描述「正向路徑」如何把 TRIZ 矛盾拆成子系統；本文件描述「反向路徑」如何跳過矛盾、直接從第一原理產生非典型架構。兩者最終在「候選方案決策中心」匯流。
...
| 文件                                            | 對齊點                                              |
| --------------------------------------------- | ------------------------------------------------ |
| `Forward_Subsystem_Discovery_Architecture.md` | 本文件為「反向路徑」的姊妹文件；兩者在決策中心匯流（本文件 §10）              |
| `AI_Agent_Architecture.md` v1.4               | Analyst Agent 的「Anti-Anchor Generation」職責即本文件 §5 |
| `RD_Design_Copilot_整合流程.md` v1.6              | 本文件補充 F0 為 TRIZ (F1) 之前的反向路徑                     |
| `RD_Design_Copilot_State_Machine.md` v1.6     | Route Artifact 狀態機見本文件 §8.1                      |
| `docs/diagrams/triz-to-scamper-flow.md` v10   | 本文件是平行反向流程的 SA 視角文字化                              |
...
> **原獨立文件，現為 E3 附錄。**
> 原檔：docs/01-define/E3x--state-machine.md
...
> **原獨立文件，現為 E3 附錄。**
> 原檔：docs/01-define/diagrams/triz-to-scamper-flow.md
